...
<%
  ...
  if(null != session){
   ...
   if(null != billId){
    ...
    OpBill bill = (OpBill)hsession.get(OpBill.class,Long.parseLong(billId));
    if(null!=bill){
   	  User consultant = bill.getConsultant();
   	  PatientInfo patientInfo = bill.getPatientInfo();
   	  OpVisit visit = bill.getOpVisits().iterator().next();
	  if(isValid){
		...
		response.sendRedirect("viewOpBills");
	  }else{	  
	%>
  <head>
   ...
   <script type="text/javascript">
    ...
    <body class="hd" onload="settime()">
     ...
     <div class="main">
   	 <div align="center" class="header">Edit Bill Details</div>
     <form name="userdata" action="editOpBill">
     <table align="center" class="inputForm" width="860px"> 
 	   <tr>
 	 	<th colspan="4" align="center">
 	 	 <input type="hidden" name="id" id="id" value="<%=bill.getId()%>" />
 	 	</th>
 	   </tr>
  	   <tr>
	   ...
	   <tr>
		...
		<td align="left"><input type="text" name="Discount" id="Discount" onkeypress="return allowNumeric(event)"/></td>
	   </tr>
	   <tr>
	   ...
	   <tr>
		...
		<td align="left"><input type="text" name="DiscountReason" id="DiscountReason" /></td>
