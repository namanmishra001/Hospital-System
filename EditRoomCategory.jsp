...
<%
 ...
 if(null != session){
   ...
   if(null != id){
	...
	RoomCategory rc = (RoomCategory)hsession.get(RoomCategory.class,Long.parseLong(id));
	if(isValid){
	  ...
	  response.sendRedirect("editRoomCategory?valid=false&msg=Room Category Updated Successfully&id="+rc.getId());
	}else{
  %>
  <head>
   ...
   <script type="text/javascript">
     function validateForm(){
       ...
       if(!isEmpty(categoryName))
    	if((categoryNameMsg=='')||(categoryNameMsg.value=='Category Name is available'))
     ...
     function lookup(inputString) {
		if(inputString.length >0 ) {
			var oldCategoyName = document.getElementById('oldCategoyName');
			if(oldCategoyName.value != inputString){
  ...
  <div class="main" style="min-height: 500px;">
  	...
  	<form method="post" name="CategoryData">
     <table align="center" class="inputForm">
	  ...
	  <tr>
	   ...
	   <td align="left"><input type="text" name="CategoryName" id="CategoryName" value="<%=rc.getName()%>" onkeyup="lookup(this.value)"/></td>
  	  </tr>
  	  <tr>
	  ...
	  <tr>
       ...
       <td align="left"><input type="text" name="ChargesPerDay" id="ChargesPerDay" value="<%=rc.getChargesperday()%>" onkeypress="return allowNumeric(event)" />(Only Numbers)</td>
      </tr>
  	  <tr>
     ...
     <input type="hidden" id="oldCategoyName" name="oldCategoyName" value="<%=rc.getName()%>"/>
    </form>
   </div>
  </body>
  <%}}else{
