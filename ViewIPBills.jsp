...
<%
   ...
   <script type="text/javascript">
	function getIPBills() {
	  ...
	  if(regddate.length > 0) {
		$.post("ViewIPBillsResult.jsp",{searchString: ""+regddate+"",consultant: ""+consultant+""},function(data){
		 $('#viewIPBillsResult').html(data).show();
		});
	  }else{
		$('#viewIPBillsResult').html("").show();
		alert("Select a Date to Display IPBill Info");
	  }
	}
  ...
  <body class="hd" onload="javascript:settime(); getData();">
   ...
   <div class="main">
    ...
    <tr align="center" style="height: 75px;vertical-align: middle;">
      <td width="350px" align="right" colspan="2">List of IPBills on
       <input type="text" readonly="readonly" id="regddate" name="regddate" value='<%=new SimpleDateFormat("dd-MM-yyyy").format(new Date())%>' size="9" class="timer"/>
   	  ...
   	  <td><input type="button" id="getData" value="Get IP Bills" onclick="getIPBills()" /></td>
    </tr>
    <tr align="center" style="height:100px;padding-top:50px;">
      ...
      <td><a href="javascript:showHideFilter()">Show/Hide Filter</a></td> 
   	...
   	<div id="viewIPBillsResult" align='center' style='margin-top:25px;min-height:500px;'></div>
   </div>
  </body>
  <%}else{
