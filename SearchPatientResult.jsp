	...
	{%>
	 <table align="center">				
	  <tr class="tr_header">
		...
		<td>City</td>
		<td>Address</td>
		<td nowrap="nowrap">Contact No1</td>
		<td nowrap="nowrap">Contact No2</td>
		<td nowrap="nowrap">Relation Type</td>
		<td nowrap="nowrap">Relative Name</td>
		<td>Consultant</td>
		<td>Total Visits</td>
		<td>Details</td>
	  </tr>
	  <%
		...
		while(it.hasNext()){
		 ...
		 <tr class='<%=(count%2==0)?"tr_even":"tr_odd"%>'>
			...
			<td nowrap="nowrap"><%=ObjectUtil.formatValue(patient.getRelativeName())%></td>
			<td nowrap="nowrap"><%=ObjectUtil.formatValue(patient.getConsultant().getName())%></td>
			<td align="center">
			 <%if(null != patient.getTotalVisits()){%>
			    <a href="javascript:fnPopUp('ViewVisitsInfo.jsp?id=<%=patient.getId()%>',400,900);"><%=patient.getTotalVisits()%></a>
			 <%}else{%>
			    <a href="javascript:fnPopUp('NewAppointment.jsp?id=<%=patient.getId()%>&billId=0&OperationType=New',400,900);" title="Click here to take a new appointment">0</a>
			 <%}%>
			</td>
		   ...
		   if(null==ipBill){
		    ...
		    <%
		     ...
		     if((null != occupancies)&&(!occupancies.isEmpty())){
		       ...
		       if(null != details)
		    	out.println("IP "+details.getRoomCategory().getName()+" "+details.getRoomnumber());
		    }%>
		</td>
		 </tr>
		<%count++;
